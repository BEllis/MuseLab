...
function generateGlobalStateSetterLines(
  globalState: Record<string, unknown> | undefined,
  options: { onlyIfMissing: boolean }
): string[] {
  const stateLines: string[] = [];
  for (const [key, value] of Object.entries(globalState ?? {})) {
    const guard = options.onlyIfMissing
      ? `if (!rt.HasKey(${escapeCiStringLiteral(key)})) `
      : "";
    if (typeof value === "boolean") {
      stateLines.push(
        `${guard}rt.SetBool(${escapeCiStringLiteral(key)}, ${value ? "true" : "false"});`
      );
    } else if (typeof value === "number" && Number.isInteger(value)) {
      stateLines.push(`${guard}rt.SetInt(${escapeCiStringLiteral(key)}, ${value});`);
    } else if (typeof value === "string") {
      stateLines.push(
        `${guard}rt.SetString(${escapeCiStringLiteral(key)}, ${escapeCiStringLiteral(value)});`
      );
    }
  }
  return stateLines;
...
function generateGlobalStateInitializer(project: Project): string {
  const lines = [`public static void ApplyStoryGlobalState(string storyId, IMuseLabRuntime rt)
    {`];
  for (const story of project.stories) {
    const stateLines = generateGlobalStateSetterLines(story.globalState, {
      onlyIfMissing: false,
    });
...
function generateMissingGlobalStateDefaults(project: Project): string {
  const lines = [`public static void ApplyMissingStoryGlobalStateDefaults(string storyId, IMuseLabRuntime rt)
    {`];
  for (const story of project.stories) {
    const stateLines = generateGlobalStateSetterLines(story.globalState, {
      onlyIfMissing: true,
...
${terminalSceneChecks}
    ...
    {
        MuseLabProjectData.ApplyMissingStoryGlobalStateDefaults(storyId, rt);
        activeStoryId = storyId;
        currentNodeId = startNodeId;
        isEnded = false;
    }
  ...
  const projectDataClass = `public static class MuseLabProjectData
{
    ...
    ${generateMissingGlobalStateDefaults(project)}
