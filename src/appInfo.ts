export const APP_NAME = "MuseLab";
export const APP_TAGLINE = "Visual Novel Designer";
export const APP_URL = "https://muselab.softweyr.co.uk/app/v1/";

export const APP_DESCRIPTION =
  "Design and play branching visual novels in a node-based editor. Build scenes with backdrops, actors, sounds, and dynamic text templates, then test your story in the player.";

export const APP_VERSION = import.meta.env.VITE_APP_VERSION;
export const GIT_DESCRIBE = import.meta.env.VITE_GIT_DESCRIBE;

export const COPYRIGHT_NOTICE = "Copyright © 2025 MuseLab. Licensed under the MIT License.";
